...
if __name__ == "__main__":
    ...
    if posts:
        ...
        output_path = Path(__file__).with_name(f"clien_today_posts_{date_suffix}.csv")
        ...
        if word_freq:
            ...
            issue_file_path = Path(__file__).with_name(f"TODAY_ISSUE_{date_suffix}.txt")
            if matching_posts:
                ...
                if save_issue_posts(top_keyword, matching_posts, issue_file_path):
                    ...
                    if summary:
                        summary_file_path = Path(__file__).with_name(
                            f"TODAY_SUMMARY_{date_suffix}.txt"
                        )
        ...
        freq_output_path = Path(__file__).with_name(
            f"clien_today_title_frequencies_{date_suffix}.csv"
        )
        ...
        word_cloud_path = Path(__file__).with_name(f"clien_today_wordcloud_{date_suffix}.png")
